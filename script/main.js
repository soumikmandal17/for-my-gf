// Animation Timeline
const animationTimeline = () => {
  const textBoxChars = document.querySelector(".hbd-chatbox");
  const hbd = document.querySelector(".wish-hbd");

  if (textBoxChars) {
    const txt = textBoxChars.textContent.trim();
    textBoxChars.innerHTML = txt.split("").map(char => `<span>${char === " " ? "&nbsp;" : char}</span>`).join("");
  }

  if (hbd) {
    const txt = hbd.textContent.trim();
    hbd.innerHTML = txt.split("").map(char => `<span>${char === " " ? "&nbsp;" : char}</span>`).join("");
  }

  const ideaTextTrans = {
    opacity: 0,
    y: -20,
    rotationX: 5,
    skewX: "15deg",
  };

  const ideaTextTransLeave = {
    opacity: 0,
    y: 20,
    rotationY: 5,
    skewX: "-15deg",
  };

  const tl = new TimelineMax();

  tl.to(".container", 0.1, {
    visibility: "visible"
  })
    // STAGE 1: Hey Rinka
    .to(".one", 0.1, { opacity: 1 })
    .from(".one", 0.7, { opacity: 0, y: 20 })
    .from(".two", 0.4, { opacity: 0, y: 20 })
    .to(".one", 0.7, { opacity: 0, y: -20 }, "+=2.5")
    .to(".two", 0.7, { opacity: 0, y: -20 }, "-=0.7")
    
    // STAGE 2: Today is 1st August
    .to(".three", 0.1, { opacity: 1 })
    .from(".three", 0.7, { opacity: 0, y: 20 })
    .to(".three", 0.7, { opacity: 0, y: -20 }, "+=2")
    
    // STAGE 3: Chat Box typing
    .to(".four", 0.1, { opacity: 1 })
    .from(".four", 0.7, { scale: 0.2, opacity: 0 })
    .from(".fake-btn", 0.3, { scale: 0.2, opacity: 0 })
    .staggerTo(".hbd-chatbox span", 0.5, { visibility: "visible" }, 0.05)
    .to(".fake-btn", 0.1, { backgroundColor: "rgb(127, 206, 248)" }, "+=1") 
    .to(".four", 0.5, { scale: 0.2, opacity: 0, y: -150 }, "+=1.2")
    
    // STAGE 4: Text Ideas progression
    .to(".five", 0.1, { opacity: 1 })
    .from(".idea-1", 0.7, ideaTextTrans)
    .to(".idea-1", 0.7, ideaTextTransLeave, "+=1.5")
    .from(".idea-2", 0.7, ideaTextTrans)
    .to(".idea-2", 0.7, ideaTextTransLeave, "+=1.5")
    .from(".idea-3", 0.7, ideaTextTrans)
    .to(".idea-3 strong", 0.5, { scale: 1.2, x: 10, backgroundColor: "rgb(21, 161, 237)", color: "#fff" })
    .to(".idea-3", 0.7, ideaTextTransLeave, "+=1.5")
    .from(".idea-4", 0.7, ideaTextTrans)
    .to(".idea-4", 0.7, ideaTextTransLeave, "+=1.5")
    .from(".idea-5", 0.7, { rotationX: 15, rotationZ: -10, skewY: "-5deg", y: 50, z: 10, opacity: 0 }, "+=0.5")
    .to(".idea-5 span", 0.7, { rotation: 90, x: 8 }, "+=0.4")
    .to(".idea-5", 0.7, { scale: 0.2, opacity: 0 }, "+=2")
    .staggerFrom(".idea-6 span", 0.8, { scale: 3, opacity: 0, rotation: 15, ease: Expo.easeOut }, 0.2)
    .staggerTo(".idea-6 span", 0.8, { scale: 3, opacity: 0, rotation: -15, ease: Expo.easeOut }, 0.2, "+=1")
    .to(".five", 0.1, { opacity: 0 }) // Hides the ideas stage completely
    
    // STAGE 5: The big reveal with balloons
    .to(".six", 0.1, { opacity: 1 })
    .staggerFromTo(".baloons img", 2.5, { opacity: 0.9, y: 1400 }, { opacity: 1, y: -1000 }, 0.2)
    .from(".girl-dp", 0.5, { scale: 3.5, opacity: 0, x: 25, y: -25, rotationZ: -45 }, "-=2")
    .staggerTo(".wish-hbd span", 0.01, { opacity: 1 })
    .staggerFrom(".wish-hbd span", 0.7, { opacity: 0, y: -50, rotation: 150, skewX: "30deg", ease: Elastic.easeOut.config(1, 0.5) }, 0.05)
    .staggerFromTo(".wish-hbd span", 0.7, { scale: 1.4, rotationY: 150 }, { scale: 1, rotationY: 0, color: "#ff69b4", ease: Expo.easeOut }, 0.05, "party")
    .from(".wish h5", 0.5, { opacity: 0, y: 10, skewX: "-15deg" }, "party")
    .staggerTo(".eight svg", 1.5, { visibility: "visible", opacity: 0, scale: 80, repeat: 3, repeatDelay: 1.4 }, 0.3)
    .to(".six", 0.5, { opacity: 0, y: 30, zIndex: "-1" }, "+=3")
    
    // STAGE 6: Replay window
    .to(".nine", 0.1, { opacity: 1 })
    .staggerFrom(".nine p", 1, ideaTextTrans, 1.2)
    .to(".last-smile", 0.5, { rotation: 90 }, "+=1");

  const replyBtn = document.getElementById("replay");
  if (replyBtn) {
    replyBtn.addEventListener("click", () => {
      tl.restart();
    });
  }
};

const fetchData = () => {
  fetch("customize.json")
    .then((data) => data.json())
    .then((data) => {
      Object.keys(data).map((customData) => {
        if (data[customData] !== "") {
          const element = document.getElementById(customData);
          if (element) {
            if (customData === "imagePath") {
              element.setAttribute("src", data[customData]);
            } else {
              element.innerText = data[customData];
            }
          }
        }
      });
    });
};

const resolveFetch = () => {
  return new Promise((resolve) => {
    fetchData();
    setTimeout(() => {
      resolve("Fetch done!");
    }, 200); 
  });
};

resolveFetch().then(() => {
  animationTimeline();
});
